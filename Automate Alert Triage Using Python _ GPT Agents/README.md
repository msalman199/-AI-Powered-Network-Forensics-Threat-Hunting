...
### Blue Team / SOC Automation Track — AI-Assisted Alert Prioritization

![Linux](https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black)
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![OpenAI](https://img.shields.io/badge/OpenAI%20GPT-Agent%20Reasoning-412991?style=for-the-badge&logo=openai&logoColor=white)
![Flask](https://img.shields.io/badge/Flask-Microservice-000000?style=for-the-badge&logo=flask&logoColor=white)
...
![SOC](https://img.shields.io/badge/SOC-Alert%20Triage-9cf?style=for-the-badge&logo=datadog&logoColor=white)
...
- [⚙️ Task 1: Set Up Environment and Install Dependencies](#️-task-1-set-up-environment-and-install-dependencies)
- [🧩 Task 2: Implement Python Scripts to Automate Triage](#-task-2-implement-python-scripts-to-automate-triage)
- [🧠 Task 3: Integrate GPT Agents for Enhanced Decision-Making](#-task-3-integrate-gpt-agents-for-enhanced-decision-making)
- [📊 Task 4: Evaluate Alert Triage Performance](#-task-4-evaluate-alert-triage-performance)
- [✅ Task 5: Verify Complete Pipeline](#-task-5-verify-complete-pipeline)
...
| # | Objective |
|---|-----------|
| 1 | Build Python scripts to automate security alert triage and classification |
| 2 | Integrate GPT-powered agents for intelligent decision-making in alert processing |
| 3 | Evaluate and measure alert triage performance using metrics and validation techniques |
| 4 | Implement a complete automated triage pipeline using open-source tools |
...
| **Python 3** | Core scripting language | ![Python](https://img.shields.io/badge/lang-Python%203-3776AB?logo=python&logoColor=white) |
| **OpenAI GPT** | AI-powered risk assessment & reasoning | ![OpenAI](https://img.shields.io/badge/AI-OpenAI%20GPT-412991?logo=openai&logoColor=white) |
| **Flask** | Lightweight service framework | ![Flask](https://img.shields.io/badge/framework-Flask-000000?logo=flask&logoColor=white) |
| **Pandas / NumPy** | Alert data manipulation & analysis | ![Pandas](https://img.shields.io/badge/data-Pandas%20%7C%20NumPy-150458) |
| **Scikit-learn** | Classification metrics & evaluation | ![Sklearn](https://img.shields.io/badge/ML-Scikit--learn-F7931E?logo=scikitlearn&logoColor=white) |
| **Matplotlib / Seaborn** | Triage performance visualization | ![Viz](https://img.shields.io/badge/viz-Matplotlib%20%7C%20Seaborn-11557C) |
| **JSON** | Alert & report data interchange format | ![JSON](https://img.shields.io/badge/format-JSON-000000?logo=json&logoColor=white) |
| **venv** | Isolated Python environment management | ![venv](https://img.shields.io/badge/env-Python%20venv-3776AB) |
...
| Requirement | Details |
|--------------|---------|
| 🐍 Python Programming | Basic Python programming knowledge |
| 🧩 JSON Data Structures | Understanding of JSON data structures |
| 🐧 Linux CLI Knowledge | Familiarity with the Linux command line |
| 🚨 Security Fundamentals | Basic knowledge of security alerts and incident response concepts |
...
## ⚙️ Task 1: Set Up Environment and Install Dependencies
...
```bash
# 📁 Create project directory and isolated environment
...
```bash
# 🗂️ Scaffold the pipeline directory structure
...
```python
# 🐍 scripts/alert_generator.py — TODO: replace synthetic alerts with live SIEM/EDR export
import json
import random
...
```python
# 🐍 scripts/basic_triage.py — weighted priority scoring engine
import json
import pandas as pd
...
> ⚠️ **TODO:** Update `gpt_model` to your target production model and store any real API key via environment variable — never commit it to `config.json`.

### 🔹 Step 3.2: Create GPT-Powered Triage Agent

```python
# 🐍 scripts/triage_agent.py — TODO: swap simulate_gpt_response() for a real OpenAI API call in production
import json
import time
import requests
from datetime import datetime
...
class GPTTriageAgent:
    ...
    def calculate_final_priority(self, alert, gpt_analysis):
        # ⚖️ Combine basic triage with GPT insights
...
```python
# 🐍 scripts/performance_evaluator.py — computes precision/recall/F1 and generates dashboards
import json
import pandas as pd
import numpy as np
...
```python
# 🐍 scripts/dashboard.py — console dashboard summarizing triage & performance results
import json
import pandas as pd
from datetime import datetime
...
```python
# 🐍 scripts/verify_pipeline.py — checks files, data integrity, and metric presence end-to-end
import json
import os
from datetime import datetime
...
> 💡 **TODO:** Attach a screenshot of your `dashboard.py` output and `triage_performance_analysis.png` as evidence of lab completion.

---

## 🗺️ MITRE ATT&CK Mapping

The synthetic alert types generated in this lab correspond to observable behaviors across the following techniques:

| Technique ID | Technique Name | Tactic | Corresponding Alert Type |
|--------------|-----------------|--------|----------------------------|
| T1204 / T1566 | User Execution / Phishing | Initial Access | "Phishing Attempt" alerts |
| T1110 | Brute Force | Credential Access | "Failed Login Attempts" alerts |
| T1068 / T1078 | Privilege Escalation / Valid Accounts | Privilege Escalation | "Privilege Escalation" alerts |
| T1071 / T1046 | Application Layer Protocol / Network Service Discovery | Command and Control / Discovery | "Suspicious Network Activity" alerts |
| T1041 | Exfiltration Over C2 Channel | Exfiltration | "Data Exfiltration" alerts |
| T1204.002 / T1055 | Malicious File Execution / Process Injection | Execution / Defense Evasion | "Malware Detection" alerts |

> 🧭 **Design Note:** The GPT agent doesn't detect these techniques directly — it re-prioritizes alerts *already* generated by upstream tools (Firewall, IDS, AV, SIEM, EDR) that map to these techniques, adding a reasoning layer on top of existing detection coverage.
...
<details>
<summary>🐍 <strong>Virtual Environment Not Activating</strong></summary>

If `source venv/bin/activate` doesn't change your prompt, confirm you ran it from inside `~/alert-triage-lab` and that `python3 -m venv venv` completed without errors first.
</details>

<details>
<summary>📦 <strong>Missing Package Errors</strong></summary>

If any script fails with `ModuleNotFoundError`, re-activate the virtual environment and re-run `pip install openai requests pandas numpy scikit-learn matplotlib seaborn flask`.
</details>

<details>
<summary>📄 <strong>File Not Found When Running Scripts</strong></summary>

The scripts use relative paths like `../data/sample_alerts.json`. Always `cd scripts` before running a script, and `cd ..` afterward, exactly as shown in each step.
</details>

<details>
<summary>🔑 <strong>Real OpenAI API Integration Fails</strong></summary>

This lab ships with `simulate_gpt = True` so no API key is required. To use a real GPT call, set `simulate_gpt = False`, add your OpenAI API key via an environment variable, and implement the API request inside `analyze_alert()`.
</details>

<details>
<summary>📊 <strong>jq Command Not Found</strong></summary>

Step 5.3 uses `jq` to count JSON records. If it's missing, install it with `sudo apt install jq -y`, or ignore the error — the script falls back to printing `N/A`.
</details>
...
You have successfully built a complete **automated alert triage system** that combines traditional rule-based classification with GPT-powered intelligent analysis.

### 🏆 Key Accomplishments

- ⚙️ Built an automated triage pipeline that classifies security alerts based on multiple weighted criteria
- 🧠 Integrated a GPT-powered agent that enhances decision-making through risk assessment and recommendation generation
- 📊 Built comprehensive metrics and visualization tools to measure and improve triage effectiveness
- 🔗 Established a complete end-to-end workflow — from alert generation through final classification and reporting
- ✅ Verified pipeline integrity with an automated file, data, and metrics validation script

### 🌍 Real-World Applications

- Automated triage significantly reduces **manual analyst workload** in high-volume SOC environments
- AI-assisted risk scoring improves **response times for critical threats** by surfacing the highest-risk alerts first
- Precision/recall/F1 tracking gives SOC leadership a **quantifiable measure** of triage system effectiveness over time
- The rule-based + LLM-reasoning hybrid pattern generalizes to **SOAR playbooks** and case-management auto-prioritization
